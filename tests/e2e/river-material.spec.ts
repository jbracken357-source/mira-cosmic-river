...
test('image-backed river loads without blocking exploration or replay', async ({ page }) => {
  ...
  const texture = page.waitForResponse(response => response.url().endsWith('/materials/river-density-v1.png'));
...
test('missing river image preserves a usable mobile scene', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.setViewportSize({ width: 390, height: 844 });
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.addInitScript(() => localStorage.setItem('mira:seen-opening', '1'));
  await page.route('**/materials/river-density-v1.png', route => route.fulfill({ status: 404, body: '' }));
  await page.goto('/?quality=low');
  await expect(page.getByTestId('explore-ui')).toBeVisible();
