...
// The tail's opacity envelope. The pull-back glimmer (up to 0.15) is the floor the
// reveal grows from — resetting to zero at the tail-reveal mark read as a pop.
function tailRamp(t: number): number {
  if (t < CINEMATIC.PULL_BACK_START) return 0;
  if (t < CINEMATIC.TAIL_REVEAL_START) {
    return clamp01((t - CINEMATIC.PULL_BACK_START) / (CINEMATIC.TAIL_REVEAL_START - CINEMATIC.PULL_BACK_START)) * 0.15;
  }
  return 0.15 + easeInOutCubic(clamp01((t - CINEMATIC.TAIL_REVEAL_START) / (CINEMATIC.TAIL_FULL - CINEMATIC.TAIL_REVEAL_START))) * 0.7;
}
...
export function resolveOpeningPose(
  ...
  if (t >= CINEMATIC.EXPLORE_MODE) {
    ...
    return { position: [...cam.EXPLORE.position], lookAt: [...cam.EXPLORE.lookAt], fov: cam.EXPLORE.fov, tailOpacity: 0.85 };
  }
  ...
  if (t < CINEMATIC.PULL_BACK_END) {
    ...
    return {
      ...
      lookAt,
      fov: lerp(CAMERA.FOV_START, CAMERA.FOV_END, eased),
      tailOpacity,
    };
  }
  if (t < CINEMATIC.FINAL_TEXT) {
    // A short hold on the far framing before the landing begins.
    return { position: [...cam.FAR.position], lookAt: [...cam.FAR.lookAt], fov: CAMERA.FOV_END, tailOpacity };
  }
  ...
  return {
    ...
    fov: lerp(CAMERA.FOV_END, cam.EXPLORE.fov, settle),
    tailOpacity,
  };
}
