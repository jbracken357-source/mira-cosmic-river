...
- **英文**：系统字体栈（Georgia / 'Times New Roman' serif 方向）；数字读数用系统等宽栈。Cinzel/Inter/JetBrains Mono 的 Google Fonts 外链已从 index.html 移除，改为本地 preload（仅 mira-serif-sc）+ swap 懒加载（mira-wenkai）。
...
- **加载策略**：`font-display: swap`；`<link rel="preload">` 只给正文/控件用的 mira-serif-sc（index.html:11）——结语字体闲置后才出现，靠 swap 懒加载，不 preload。回退栈完整，字体未到时系统 serif 接管，不阻塞渲染。
...
- 页面语言同步：index.html 初始 `lang="zh-CN"`（index.html:2，与 store 默认中文一致，首帧与读屏器初始公告即为中文）；`App.tsx` 的 effect 在切换后把 `<html lang>` 同步为 `zh-CN` / `en`。
- 完整开场的提前进入按钮：新增翻译键 `enterEarly`（en "Enter early" / ch "提前进入"，translations.ts:36,108），替换 CinematicOverlay.tsx 的硬编码 "Skip"；可见文本即可访问名，不再重复 aria-label；标识符遵守领域语言（直达/提前进入），存量 testid `skip-cinematic` 未改名。
...
- 可访问名称：replay-opening `aria-label={t.replayOpening}`、pause-toggle `aria-label` 跟随播放态、tail-hint `aria-label={t.tailHint}`、info-card-close `aria-label={t.tonightClose}`；其余按钮（含 skip-cinematic）以可见文本命名。
- 焦点态：元素级统一焦点环（src/index.css，`button/input/[role=button]:focus-visible` 用 nebula-violet 2px outline），不再逐按钮堆类；`.glass-button:focus-visible` 旧规则已删（与元素级规则重复）。
...
- Esc 与焦点：InfoCards.tsx——打开时（ref 回调中）记住 `document.activeElement` 并把焦点移上卡片（`tabIndex={-1}`），关闭后焦点回到触发元素；卡间直切（miraA→miraB）时新卡在旧卡退场后才挂载，聚焦放在 ref 回调而非 effect，焦点落在新卡不掉回 body，关闭仍回到最初入口。Esc 监听器仅在卡片打开时挂在 window。卡片为非模态 `role="region"` + `aria-label`，不锁焦点。
- e2e：interface.spec.ts "a star trigger opens its card; Esc closes it and focus returns to the trigger"（断言 `toBeFocused` 双向）、"switching directly between cards keeps focus on the new card; Esc returns to the original trigger"、"the close control has an accessible name and closes with the keyboard"。
...
- e2e 四档：320×568、390×844、430×932、844×390 横屏（interface.spec.ts 末段参数化）——pause/replay/language/tail-hint 可见，卡片打开后关闭按钮可见且可点。截图：
  ...
  - `evidence/interface-20-card-open.png`（390×844 信息卡展开态：标题、描述、脉动相位行、44px 关闭 ×）
  - `evidence/interface-20-epilogue.png`（结语实渲；e2e 同时断言 `document.fonts.check('24px "Mira WenKai"', '星')` 为 true，证明文楷字体真正加载而非宋体回退）
...
- 操作提示：interactionHint/interactionHintMobile 在观看者首次操作场景后淡出退场——监听落在 canvas 上的 pointerdown/wheel/touchstart（全部 passive；UI 按钮上的点击不算场景操作，CinematicOverlay.tsx ExploreUI）；已掌握状态写 `localStorage` 键 `mira:learned-controls`（照 `mira:seen-opening` 先例），回访不再重现；footer 留常驻低透明 `interaction-hint-recall` 小按钮（44px，aria-label 为提示文案）可唤回。MilestoneHint 一次性逻辑未动。
- e2e："the first scene manipulation retires the hint; a quiet entry brings it back"（含 localStorage 写入断言）、"a returning visit does not show the hint again, but the recall entry stays"。

## 回归与门禁

- `tests/e2e/interface.spec.ts`：17 例全过（键盘 4、触控/焦点 2、语言 2、提示 2、动效 1、viewport 4、证据截图 2）。
- `tests/unit/fontSubset.spec.ts` + `animationTiming.spec.ts`：5 例全过。
- 全量 `npx playwright test`（串行）：272+ 例全过，见提交时门禁输出。
